...
In the beginning, the Wikipedia extractor prints out how many processes it will use for the extraction: `INFO: Using 7 extract processes.` After that, a list of all extracted articles is printed out. As soon as some articles are extracted, you can abort the process and continue with step 3.
...
#### Input file format
The input files ingested by `cv-sentence-extractor` are following a JSON format, based on what the Wikipedia extractor outputs. The format is simple:
...
The file must follow the following directory structure and filename: `AA/wiki_XX`, with `AA` an alphanumerical numbering and `XX` two numbers for different files. The file should not have a filetype.
